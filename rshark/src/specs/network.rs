use std::{default, fmt::{Display, Write}};
use std::fmt;


use log::info;

use crate::{common::{IPv4Address, Protocol, Reader}, files::{Frame, Initer, PacketContext}};
...
impl IPv4 {
    pub fn _source_ip(p: &IPv4) -> String{
        String::from("sourc-asd")
    }
    pub fn _target_ip(p: &IPv4) -> String{
        String::from("sourc-asd")
    }
}
...
impl Initer<IPv4> for IPv4 {
    fn new() -> IPv4 {
        IPv4{protocol: Protocol::IPV4, ..Default::default()}
...
impl crate::files::Visitor for IP4Visitor {
    fn visit(&self, frame: &Frame, reader: &Reader){
        ...
        let total_len = packet.read(reader, Reader::_read16_be, None);
        let identification = packet.read(reader, Reader::_read16_be, None);
        let flag  = packet.read(reader, Reader::_read16_be, None);
        let ttl = packet.read(reader, Reader::_read8, None);
