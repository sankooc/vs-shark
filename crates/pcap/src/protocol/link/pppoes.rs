...
fn payload(protocol: u16, reader: &mut Reader) -> Result<Vec<Field>> {
    ...
    match protocol {
        0x0021 => {
            if reader.left() >= 1 {
                let version_ihl = reader.read8()?;
                let version = (version_ihl >> 4) & 0x0F;
                let ihl = version_ihl & 0x0F;
                field_back_format!(list, reader, 1, format!("IPv4 Version: {}, Header Length: {} bytes", version, ihl * 4));

                if reader.left() >= 1 {
                    let dscp_ecn = reader.read8()?;
                    let dscp = (dscp_ecn >> 2) & 0x3F;
                    let ecn = dscp_ecn & 0x03;
                    field_back_format!(list, reader, 1, format!("DSCP: {}, ECN: {}", dscp, ecn));

                    if reader.left() >= 18 {
                        let total_length = reader.read16(true)?;
                        field_back_format!(list, reader, 2, format!("Total Length: {} bytes", total_length));

                        let identification = reader.read16(true)?;
                        field_back_format!(list, reader, 2, format!("Identification: {:#06x}", identification));

                        let flags_fragment = reader.read16(true)?;
                        let flags = (flags_fragment >> 13) & 0x07;
                        let fragment_offset = flags_fragment & 0x1FFF;
                        field_back_format!(list, reader, 2, format!("Flags: {:#04x}, Fragment Offset: {}", flags, fragment_offset));

                        let ttl = reader.read8()?;
                        field_back_format!(list, reader, 1, format!("Time to Live: {}", ttl));

                        let protocol = reader.read8()?;
                        field_back_format!(list, reader, 1, format!("Protocol: {}", protocol));

                        let checksum = reader.read16(true)?;
                        field_back_format!(list, reader, 2, format!("Header Checksum: {:#06x}", checksum));

                        let source_ip = reader.read_ip4()?;
                        field_back_format!(list, reader, 4, format!("Source Address: {}", source_ip));

                        let dest_ip = reader.read_ip4()?;
                        field_back_format!(list, reader, 4, format!("Destination Address: {}", dest_ip));

                        field_rest_format!(list, reader, format!("Options and Data: {} bytes", reader.left()));
                    } else {
                        // 数据不足
                        reader.forward(reader.left());
                        field_rest_format!(list, reader, "Incomplete IPv4 Header".to_string());
                    }
                } else {
                    // 数据不足
                    reader.forward(reader.left());
        ...
        0x0057 => {
            // IPv6
            if reader.left() >= 40 {
                let version_tc_fl = reader.read32(true)?;
                let version = (version_tc_fl >> 28) & 0x0F;
                let traffic_class = (version_tc_fl >> 20) & 0xFF;
                let flow_label = version_tc_fl & 0xFFFFF;
                field_back_format!(
                    list,
                    reader,
                    4,
                    format!("IPv6 Version: {}, Traffic Class: {}, Flow Label: {:#08x}", version, traffic_class, flow_label)
                );

                let payload_length = reader.read16(true)?;
                field_back_format!(list, reader, 2, format!("Payload Length: {} bytes", payload_length));

                let next_header = reader.read8()?;
                field_back_format!(list, reader, 1, format!("Next Header: {}", next_header));

                let hop_limit = reader.read8()?;
                field_back_format!(list, reader, 1, format!("Hop Limit: {}", hop_limit));

                let source_ip = reader.read_ip6()?;
                field_back_format!(list, reader, 16, format!("Source Address: {}", source_ip));

                let dest_ip = reader.read_ip6()?;
                field_back_format!(list, reader, 16, format!("Destination Address: {}", dest_ip));

                field_rest_format!(list, reader, format!("Extension Headers and Data: {} bytes", reader.left()));
            } else {
                field_rest_format!(list, reader, "Incomplete IPv6 Header".to_string());
            }
        }
        0xc021 => {
            if reader.left() >= 4 {
